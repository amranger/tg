...
<div id="search-form">
	<form action="<?php bloginfo('url'); ?>" id="searchform" method="get">
		<input type="text" id="searchinput" name="s" value="<?php echo $searchPhrase; ?>">
	</form>
</div>
<?php
	}
  ...
  /*Saves the settings. */
    function update($new_instance, $old_instance){
		...
		$instance['searchPhrase'] = stripslashes($new_instance['searchPhrase']);
  ...
  /*Creates the form for the widget in the back-end. */
    function form($instance){
		...
		$searchPhrase = htmlspecialchars($instance['searchPhrase']);

		#Search phrase
		echo '<p><label for="' . $this->get_field_id('searchPhrase') . '">' . 'Search phrase:' . '</label><input type="text" class="widefat" id="' . $this->get_field_id('searchPhrase') . '" name="' . $this->get_field_name('searchPhrase') . '" value="'.$searchPhrase.'"  /></p>';
	}
