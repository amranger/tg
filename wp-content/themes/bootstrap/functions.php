...
function remove_footer_admin () {
    echo 'Thank you for choosing <a href="http://www.telegraphicsinc.com">Telegraphics, Inc</a>. <br />Email us at <a href="mailto:support@telegraphicsinc.com">support@telegraphicsinc.com</a> for technical support.';
}
	...
	<li <?php comment_class(); ?>>
		<article id="comment-<?php comment_ID(); ?>" class="clearfix">
			<div class="comment-author vcard row-fluid clearfix">
				<?php if(get_comment_author() == 'Joe Martin'): ?>
				<div class="span2">
						<?php $employee = wp_get_attachment_image_src( get_field('e_image', 50),'blog-employee'); ?>
						<img src="<?=$employee[0];?>" class="img-polaroid" />				
				</div>
				<?php elseif(get_comment_author() == 'Aaron Ranger'): ?>
				<div class="span2">
						<?php $employee = wp_get_attachment_image_src( get_field('e_image', 52),'blog-employee'); ?>
						<img src="<?=$employee[0];?>" class="img-polaroid" />				
				</div>
				<?php else: ?>
				...
				<div class="span10 comment-text">
					<h4><?php printf(__('%s','bonestheme'), get_comment_author_link()) ?> <time datetime="<?php echo comment_time('Y-m-j'); ?>"><?php comment_time('F jS, Y'); ?></time></h4>
...
{ 
    ...
    wp_register_style( 'tg-custom', get_template_directory_uri() . '/style-custom.css', array(), '1.0', 'all' );
    wp_register_style( 'font-awesome', get_template_directory_uri() . '/library/css/font-awesome.min.css', array(), '1.0', 'all' );
    
    wp_enqueue_style( 'bootstrap' );
    wp_enqueue_style( 'bootstrap-responsive' );
    wp_enqueue_style( 'wp-bootstrap');
    wp_enqueue_style( 'tg-custom');
...
function theme_js(){
  ...
  // wp_register_script('hscroll', get_template_directory_uri().'/library/js/hscroll.js');
  ...
  // wp_enqueue_script('hscroll', array('jQuery'), '1.1', true);
}
add_action('wp_enqueue_scripts', 'theme_js');


require_once(TEMPLATEPATH . '/includes/post_thumbnails.php');
...
require_once(TEMPLATEPATH . '/includes/mobile.php');
...
function relative_time() {
    ...
    elseif ($delta > (24*60*60) && $delta < (2*24*60*60)){
    	echo floor(($delta/(24*60*60))) . ' day ago';
    }
    elseif ($delta > (2*24*60*60) && $delta < (6*24*60*60)){
    	echo floor(($delta/(24*60*60))) . ' days ago';
    }
    else {
        echo "on "; echo the_time('D, M j, Y');
    }
}
